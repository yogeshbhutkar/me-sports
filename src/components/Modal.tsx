...
export default function Modal() {
  ...
  useEffect(() => {
    try {
      ...
      fetchSingleArticle(params.id ?? "").then((res) => {
        setArticle(res);
        setLoading(false);
      });
    } catch (err) {
                  ...
                  <div className=" px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div className="sm:flex  sm:items-start">
                      <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        <img
                          className="items-center object-cover h-72 w-full pb-2 rounded-xl"
                          src={article?.thumbnail}
                          alt="Thumbnail"
                        />
                        <Dialog.Title
                          as="h3"
                          className="text-base font-bold mt-2 leading-6 text-gray-300"
                        >
                          {article?.title}
                        </Dialog.Title>
                        <div className="mt-1">
